...
class MemorySystemConfig(BaseModel):
    ...
    llm_name: str = Field(
        "gpt-4.1-mini", description="Name of the LLM model to be used."
    )
    openai_api_key: Optional[str] = Field(
        None, description="API key for OpenAI-compatible gateway used by memory LLM."
    )
    openai_base_url: Optional[str] = Field(
        None, description="Base URL for OpenAI-compatible gateway used by memory LLM."
    )
...
class SemanticRecordPayload(BaseModel):
    summary: str = Field(
        "", description="A brief summary of the SemanticRecord/EpisodicRecord."
    )
    detail: Union[str, dict] = Field(
        "", description="Detailed information about the SemanticRecord."
    )
    tags: Optional[Iterable[str]] = Field(
        None, description="Tags associated with the SemanticRecord."
    )
    is_abstracted: bool = Field(
        False, description="Indicates if the SemanticRecord is abstracted."
    )
...
class EpisodicRecordPayload(BaseModel):
    ...
    detail: Union[str, dict] = Field(
        "", description="Detailed information about the EpisodicRecord."
    )
    stage: str = Field("", description="Stage of the EpisodicRecord.")
    tags: Optional[Iterable[str]] = Field(
        None, description="Tags associated with the EpisodicRecord."
    )
...
class ProceduralRecordPayload(BaseModel):
    ...
    detail: Union[str, dict] = Field(
        "", description="Detailed information about the ProceduralRecord."
    )
    steps: Optional[List[str]] = Field(
        None, description="Steps for the ProceduralRecord."
    )
    code: Optional[str] = Field(
        None, description="Code snippet for the ProceduralRecord."
    )
    tags: Optional[Iterable[str]] = Field(
        None, description="Tags associated with the ProceduralRecord."
    )
...
class MemorySystem(ABC):
    ...
    @abstractmethod
    def add(
        self, memories: List[Union[SemanticRecord, EpisodicRecord, ProceduralRecord]]
    ) -> bool: ...

    @abstractmethod
    def update(
        self, memories: List[Union[SemanticRecord, ProceduralRecord]]
    ) -> bool: ...
    ...
    def query(
        ...
        Tuple[float, List[Union[SemanticRecord, EpisodicRecord, ProceduralRecord]]]
    ]: ...
