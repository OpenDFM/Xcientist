...
class FAISSMemorySystem(MemorySystem):
    def __init__(self, **kwargs):
        ...
        if self.memory_type == "semantic":
            self.global_cidmap2semrec: Dict[int, SemanticRecord] = (
                {}
            )  # {cluster_id: SemanticRecord}, Only updated when abstracted semantic records are processed
    ...
    def instantiate_epi_record(self, **kwargs) -> EpisodicRecord:
        ...
        record.embedding = self.vector_store._embed(
            _transfer_dict_to_semantic_text(record.detail)
        )
        return record
    ...
    def get_last_k_records(
        ...
        Union[List[SemanticRecord], List[EpisodicRecord], List[ProceduralRecord]], int
    ]:
        if k >= self.size:
            return (
                [record.to_dict() for record in self.vector_store.meta.values()],
                self.size,
            )
            ...
            return (
                [self.vector_store.meta[fid].to_dict() for fid in sorted_fids[:k]],
                k,
            )
    ...
    def add(
        self,
        memories: List[Union[SemanticRecord, EpisodicRecord, ProceduralRecord]] = None,
        agent_id: str = "",
    ) -> bool:
        try:
            self.vector_store.add(
                memories, agent_id=agent_id
    ...
    def update(
        self, memories: List[Union[SemanticRecord, ProceduralRecord]] = None
    ) -> bool:
        try:
            self.vector_store.update(
                memories
            )  # Update new memory to FAISS vectorstore.
            return True
        except Exception as e:
    ...
    def upsert_normal_records(
        self, records: List[Union[SemanticRecord, ProceduralRecord]], agent_id: str = ""
    ) -> None:
        for record in records:
            result = self.get_nearest_k_records(
                record, k=1, threshold=0.8, agent_id=agent_id
            )
            if result is not None and len(result) > 0:
                ...
                if isinstance(record, SemanticRecord):
                    ...
                    # Persist updated text into the vector index + meta.
                    self.update([nearset_record])
    ...
    def query(
        ...
        try:
            ...
            logger.warning(
                "Memory query failed (type=%s, method=%s, agent_id=%s): %r",
                getattr(self, "memory_type", ""),
                method,
                agent_id,
                e,
            )
    ...
    async def abstract_episodic_records(
        self, epi_records: List[EpisodicRecord], consistency_threshold: float = 0.8
    ) -> Tuple[List[SemanticRecord], Dict[int, SemanticRecord]]:
        # TODO: Debug
        assert (
            self.memory_type == "episodic"
        ), "Clustering is only supported for episodic memory type."

        cidmap2mid: Dict[int, List] = defaultdict(
            list
        )  # {cluster_id: episodic_record_id}
        midmap2epirec: Dict[str, EpisodicRecord] = (
            {}
        )  # {episodic_record_id: EpisodicRecord}
        ...
        clusters_sorted = sorted(
            self.cluster_machine.cidmap2cluster.values(),
        ...
        for cl in clusters_sorted:
            ...
            if (
                cl.kind.value == "PMC"
                and cl.avg_pairwise_cos() >= consistency_threshold
                and cl.id in updated_cluster_id
            ):
                    ...
                    episodic_notes.append(
                        "\n".join(
                            [
                                ...
                                f"Detail: {detail_text}," f"Tags: {tags_text}",
                            ]
                        )
                    )
    ...
    def upsert_abstract_semantic_records(
        self,
        sem_records: List[SemanticRecord],
        cidmap2semrec: Dict[int, SemanticRecord],
        agent_id: str = "",
    ) -> None:
        add_list = []
        update_list = []
    ...
    def get_nearest_k_records(
        ...
        try:
            results = self.vector_store.query(
                query_text, method=method, limit=k, filters=filters, agent_id=agent_id
            )
        except Exception as e:
        ...
        if threshold is not None:
            ...
            if thr is not None:
                results = [
                    (score, rec) for score, rec in results if float(score) >= thr
                ]
        return results
