...
def main() -> None:
    ...
    results = semantic_memory_store.query(
        "augmentations robustness weather", method="embedding", limit=2
    )
    print(
        f"SemanticRecord query test results: {[{'score': r[0], 'record': r[1].to_dict()} for r in results]}"
    )

    print("""--------------------Memory query(bm25) test--------------------""")
    results = semantic_memory_store.query(
        "augmentations weather", method="bm25", limit=2
    )
    print(
        f"SemanticRecord query test results: {[{'score': r[0], 'record': r[1].to_dict()} for r in results]}"
    )
    ...
    results = semantic_memory_store.query(
        "augmentations weather", method="overlapping", limit=2
    )
    print(
        f"SemanticRecord query test results: {[{'score': r[0], 'record': r[1].to_dict()} for r in results]}"
    )
    ...
    print(
        f"SemanticMemoryStore load test: {another_semantic_memory_store.load('sem_test')}"
    )
    ...
    print(
        f"EpisodicMemoryStore load test: {another_episodic_memory_store.load('epi_test')}"
    )
    ...
    print(
        f"SemanticRecord delete test: {another_semantic_memory_store.delete(delete_ids)}"
    )
    ...
    print(
        f"SemanticRecord fetch test results: {[r.to_dict() for r in fetched_records]}"
    )
    ...
    print(
        f"Nearest ProceduralRecords: {[r[1].to_dict() for r in nearest_proc_records]}"
    )
