...
### Value rule(must be followed to form "value")
1. **Insights:** you should always summary and provide insights. You are a **Scientific Literature Analysist**. You should not only extract information from the original text. You should give out your deep understanding of the paper.
...
3. **Add-Keynote:** you should add keynotes to provide key aspect or insights in the value(for example, add key innovations of a framework when extract the "method" rather than barely give the method name)
4. **Add-Example:** you should add corresponding examples of the extracted value, expecially when the paper mentions give out some examples. You can also add some appropriate examples by yourself(for example, add specific example when extracting the "limitations" of current methods).
5. **example:**
    -**negative example:**
    -"proposed_solution": {{
