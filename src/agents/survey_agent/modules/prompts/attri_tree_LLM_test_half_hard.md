...
3. Each extracted field value must be an object with these subfields:
   ...
   - "type": data type of item "value(string, float, integer)
   ...
   - "provenance": a structured pointer to where the evidence came from, formatted as an object with any available fields from {{"section_header": string | null, "page": integer | null, "original text": string | null}}. Use null for unknown or uncertain. For "original text", you can include plenty of content, as long as they are the necessary original text to support your extracting content.
