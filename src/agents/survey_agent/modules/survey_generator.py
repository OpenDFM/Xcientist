...
class SurveyGenerator:
            ...
            for retry_time in range(
                ...
                try:
                    if self.config.BasicInfo.debug and retry_time > 0:
                        self.logger.info(f"Retry time {retry_time}, with cumulative error info: {err_info.get_errors_text()} in OUTLINE GENERATION-DRAFT")
    ...
    def review_and_revise_survey(self, draft, outline):
