# script_original_text_replace.py
...
# -------------------------
# Build sentence lines for context (like your original script did)

# -------------------------
# Prompt: request exactly one replacement edit (or done).
# IMPORTANT: model MUST output one JSON object ONLY (no prose), with exactly one of:
# 1) {"action":"replace","originalText":"<exact substring>","newText":"<replacement>"}
# 2) {"action":"done"}  -> means no more edits needed
prompt = """
You are a revise assistant. The section content of a survey concerning {topic} is provided below. You must propose at most ONE exact textual substitution per response.
If you think the document requires changes, choose one that you think is most important to address next, and output ONE JSON object (and nothing else) of the form:
{{
    "action":"replace", 
...
- The section text should have at least 3000 words. If not enough, expand it with new content or insights.
- Correct citation like "**Title**" to the canonical form <Title>(like <Attention is All you need>) if needed.
- Remove any obviously irrelevant literature, but ensure to retain all correctly cited literature.
...
- The content of the section must be consistent with its title. Delete or modify any content that is off-topic.
...
- Any additions that are awkward, inconsistent with the paragraph logic, or beyond the scope of the content are strictly prohibited
"""
...
# -------------------------
# Call the model (single-edit policy)
text = original_text
for refine_time in range(1000):
    print(f"======= REFINE TIME {refine_time+1} =======")
    print("\n\n")
    try:
        resp = client.chat.completions.create(
                model=MODEL,
                messages=[{"role":"user","content": prompt.format(text = text, citations=citations, topic = "Multimodal Large Language Models")}],
                temperature=0
            )
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError("Failed to parse JSON object from model response.") from e
        ...
        if action == "done":
            print("Model indicates no edit is required (action=done). Exiting.")
            sys.exit(0)

        if action != "replace":
            raise RuntimeError("Model must return action 'replace' or 'done'.")

        original_sub = parsed.get("originalText")
        new_sub = parsed.get("newText")
        if not isinstance(original_sub, str) or not isinstance(new_sub, str):
            raise RuntimeError("Both originalText and newText must be strings.")

        # 3) search for exact occurrence of original_sub in the full original_text
        idx = text.find(original_sub)
        if idx == -1:
            # Not found -> abort and print helpful diagnostic (we require exact originalText)
            print("ERROR: The model-provided originalText was NOT found exactly in the document.")
            print("Length of originalText:", len(original_sub))
            print("Snippet (first 200 chars):")
            print(original_sub)
            # Optionally, show best fuzzy match location to help user/regenerate
            import difflib
            seq = difflib.SequenceMatcher(None, text, original_sub)
            ...
            if best and best.size>5:
                print("Best matching segment in document at pos", best.a, "length", best.size)
                print("Matched text:", text[best.a:best.a+best.size])
            raise RuntimeError("Original substring not found; the LLM must return a precise substring.")

        # 4) apply the replacement (first occurrence)
        start = idx
        end = idx + len(original_sub)
        final_text = text[:start] + new_sub + text[end:]

        # 5) print results
        print("=== APPLYING SINGLE REPLACEMENT ===")
        print("Original substring found at positions:", start, end)
        print("original substring preview:", repr(original_sub[:120]))
        if (refine_time+1) % 20 == 0:
            print("\n=== NEW TEXT (preview first 1000 chars) ===")

            print(final_text)
        text = final_text  # update for next iteration

        # time.sleep(1)
    except Exception as e:
        print("ERROR during refinement:", str(e))
# If you want to continue iteratively, you could loop: feed final_text back to the model for the next single edit.
