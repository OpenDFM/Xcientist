...
# Lazy-loaded memory config
_memory_config = None

def _get_mem_cfg():
    """Get memory config, loading from YAML on first call."""
...
def _get_shared_memory_root() -> str:
    return str(_get_mem_cfg().get("shared_dir", "") or "")
...
class _SharedMemoryStores:
    ...
    def _try_init(self) -> None:
        with self._lock:
            ...
            if not bool(_get_mem_cfg().get("enabled", True)):
            ...
            def _load_or_empty(memory_type: str):
                llm_cfg = {}
                try:
                    llm_cfg = get_openai_config(model=_get_mem_cfg().get("llm_name", "glm-5"))
                except Exception:
                    llm_cfg = {}
                ...
                store = FAISSMemorySystem(
                    ...
                    model_path=str(
                        _get_mem_cfg().get("embedding_model_path", "./.cache/all-MiniLM-L6-v2")
                    ),
                    llm_name=str(_get_mem_cfg().get("llm_name", "glm-5")),
            ...
            try:
                ...
                llm_model = OpenAIClient(
                    model=str(_get_mem_cfg().get("llm_name", "glm-5")),
...
def maybe_augment_tool_result(
    ...
    if (not hint_text) and (not (_get_mem_cfg().get("tool_logs_enabled", False) and log_text)):
        return result

    out = result
    if hint_text:
        out = _attach_hint(out, hint_text)
    if _get_mem_cfg().get("tool_logs_enabled", False) and log_text:
...
async def writeback_current_task_async(
    ...
    if not _STORES.is_ready():
        return
    if not bool(_get_mem_cfg().get("writeback_enabled", True)):
        return
    ...
    max_slots = int(_get_mem_cfg().get("max_slots_per_task", 100))
...
def _retrieve_memory_hints_with_log(
    ...
    preferred_method = (
        str(_get_mem_cfg().get("query_method", "embedding")).strip().lower()
        if str(_get_mem_cfg().get("query_method", "embedding")).strip().lower()
...
def retrieve_memory_for_worker_prompt(
    ...
    """
    cfg = _get_mem_cfg()
    if not bool(cfg.get("enabled", True)) or not bool(cfg.get("prompt_injection_enabled", True)):
        return ""
    if not _STORES.is_ready():
        return ""
