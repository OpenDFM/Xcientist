...
# =============================================================================
# Ablation Experiment Schemas (for Component-Level MCTS Integration)
# =============================================================================


class AblationResult(BaseModel):
    """
    Result of an ablation experiment for a single component.

    This schema is designed for integration with LigAgent's symbolic memory system.
    """

    result: str = Field(
        description="Ablation result: positive, negative, or inconclusive"
    )
    metric: str = Field(description="The metric used for evaluation")
    value: str = Field(description="The measured value or change")
    confidence: float = Field(
        description="Confidence level of the result (0-1)", ge=0.0, le=1.0
    )
    analysis: str = Field(
        description="Detailed analysis of why this component worked or didn't work"
    )
    method_context: str = Field(
        description="Description of the method variant actually used for this component experiment"
    )
    experiment_config: Optional[Dict[str, Any]] = Field(
        default=None, description="Configuration used for this ablation experiment"
    )
    supporting_diagnostics: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Optional nested diagnostic evidence tied to this component",
    )
    follow_up_required: bool = Field(
        default=False,
        description=(
            "Whether the current evidence is still insufficient and more ablation "
            "work is required before the component can be considered fully validated"
        ),
    )
...
class ExperimentSummary(BaseModel):
    """
    Summary of the overall experiment results.
    """

    feasible: bool = Field(description="Whether the idea is deemed feasible based on experiments")
    confidence: float = Field(
        description="Overall confidence in the verdict (0-1)", ge=0.0, le=1.0
    )
    key_findings: List[str] = Field(
        default_factory=list, description="List of key findings from experiments"
    )


class AblationResults(BaseModel):
    """
    Container for all ablation experiment results.

    This is the primary output format for integration with LigAgent's
    symbolic memory system.
    """

    components: Dict[str, AblationResult] = Field(
        default_factory=dict, description="Ablation results per component"
    )
    summary: ExperimentSummary = Field(description="Overall experiment summary")
    metadata: Optional[Dict[str, Any]] = Field(
        default=None, description="Additional metadata about the experiments"
    )
